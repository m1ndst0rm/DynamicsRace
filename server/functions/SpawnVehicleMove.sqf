...
DYN_RACE_SpawnVehicleMove =
{
	...
	for [{_i = 0},{_i<(count _map_vehicles)}, {_i=_i+1}] do
	{
		...
		if(_lcl_vehicle_spawn_id == _vehicles_spawn_id) then
		{
			...
			if(_vehicles_to_spawn < _vehiclesPerSpawn) then
			{
				_vehicles_to_spawn = _vehiclesPerSpawn;
			};
	...
	{
		_vehiclePosition = [_marker_from, 0, _vehicles_to_spawn*25, 0, 1, 50*(pi/180), 0] call BIS_fnc_findSafePos;
