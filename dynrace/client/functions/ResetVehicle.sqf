/* DYN_fnc_ResetVehicle: Reset the vehicle from a local player.
...
*/
private ["_resetPos","_resetDir","_closes_road","_closest_pos","_nearRoads","_road","_currentCheckpointId","_currentCheckpoint","_previousCheckpoint","_previousCheckpointId","_text","_vehicle"];
["Ressetting vehicle %1", vehicle player] call BIS_fnc_logFormat;
	...
	[] spawn
	{
		if(time - DYN_PLAYER_VEHICLE_RESET_TIME > 5) then
		{
			...
			{		
				...
				_resetPos = [position player, 0, 15, 0, 1, direction player, 0] call BIS_fnc_findSafePos;
				_resetDir = direction player;
			};
