...
DYN_RACE_SpawnLocalRaceChecks =
{
	...
	[] spawn {
		...
		{
			//Finish time
			if ( !(DYN_RACE_FINISHTIME == -1) && diag_tickTime - _lastCheck >= 1 ) then
			{
			...
			//Player in vehicle
			//TODO: Check possebility of removing get out
			...
			if(DYN_RACE_MUST_STAY_ON_ROAD_LOCAL && DYN_RACE_ELAPSED_TIME > 15) then
			{
				...
				else
				{
					...
					{//Display reset timer
						...
						if(_time - DYN_RACE_LAST_WARNING > 1) then
						{
			...
			//Vehicle fliped
			if(vehicle player != player && false) then
			{
			...
			//TODO: Evalutate wether this isn't to much of an performance hit
			sleep 0.1;
		};
	};
};